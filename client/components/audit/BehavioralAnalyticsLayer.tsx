...
export default function BehavioralAnalyticsLayer() {
  ...
  const userProfiles = [
    ...
    },
    {
      user: 'ops_manager',
      profile: [
        { metric: 'Login Time', value: '07:00 - 19:00 UTC' },
        { metric: 'Avg Session Duration', value: '10-12 hours' },
        { metric: 'Daily Actions', value: '30-50' },
        { metric: 'Escalation Frequency', value: 'Rare' },
        { metric: 'Data Export Frequency', value: 'Daily' }
      ]
    },
    {
      user: 'transport_analyst',
      profile: [
        { metric: 'Login Time', value: '06:00 - 22:00 UTC' },
        { metric: 'Avg Session Duration', value: '8-10 hours' },
        { metric: 'Daily Actions', value: '40-60' },
        { metric: 'Escalation Frequency', value: 'Never' },
        { metric: 'Data Export Frequency', value: 'Bi-weekly' }
      ]
    }
  ];
  ...
  const filteredAnomalies = searchUser ? anomalies.filter(a => a.user.toLowerCase().includes(searchUser.toLowerCase())) : anomalies;
  const filteredProfiles = searchUser ? userProfiles.filter(p => p.user.toLowerCase().includes(searchUser.toLowerCase())) : userProfiles;
  const filteredThreats = searchUser ? insiderThreats.filter(t => t.user.toLowerCase().includes(searchUser.toLowerCase())) : insiderThreats;
      ...
      <div className="rounded-xl border border-border/50 p-6 bg-card/50">
        ...
        <div className="space-y-4">
          {filteredAnomalies.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">No anomalies found for this user</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border/50 p-6 bg-card/50">
        ...
        <div className="space-y-3">
          {filteredThreats.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">No threat scores found</p>
          )}
        </div>
      </div>
