...
export const AlarmTable: React.FC<AlarmTableProps> = ({
  ...
  const ColumnHeader = ({ label, column }: { label: string; column: SortConfig['key'] }) => (
    <th
      ...
      className="px-1 py-1 text-left text-xs font-semibold text-gray-700 bg-gray-50 border-b border-gray-200 cursor-pointer hover:bg-gray-100 transition"
    >
      {label}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full border-collapse text-xs">
          <thead className="sticky top-0 z-10">
            <tr>
              <th className="px-1 py-1 text-left bg-gray-50 border-b border-gray-200 w-8">
                <input
                  type="checkbox"
              ...
              {expertMode && <th className="px-2 py-1 text-left text-xs font-semibold text-gray-700 bg-gray-50 border-b border-gray-200">Vendor Code</th>}
              ...
              <React.Fragment key={alarm.globalAlarmId}>
                ...
                >
                  ...
                  {visibleColumns.has('severity') && (
                    <td className="px-2 py-1 cursor-pointer hover:bg-gray-100" onClick={() => onAlarmSelect(alarm)}>
                      <div className="flex items-center gap-1">
                        <span className="text-sm">{getSeverityIcon(alarm.severity)}</span>
                        <span className={`font-semibold text-xs ${getSeverityTextColor(alarm.severity)}`}>
                  ...
                  {visibleColumns.has('title') && (
                    <td className="px-2 py-1 font-medium text-gray-900 max-w-xs truncate text-xs">
                      {alarm.title}
                    </td>
                  )}
                  ...
                  {visibleColumns.has('sourceSystem') && (
                    <td className="px-2 py-1">
                      <span className="px-1 py-0.5 bg-blue-100 text-blue-800 text-xs font-semibold rounded">
                        {alarm.sourceSystem}
                      </span>
                    </td>
                  )}
                  ...
                  {expertMode && (
                    <td className="px-2 py-1 text-xs text-gray-500 font-mono">
                ...
                {expandedAlarmId === alarm.globalAlarmId && (
                  <tr className="border-b border-gray-200 bg-gray-50">
                    <td colSpan={expertMode ? 10 : 9} className="px-3 py-2">
                      <div className="space-y-2">
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <h4 className="text-xs font-semibold text-gray-600 mb-1">Description</h4>
                            <p className="text-xs text-gray-800">{alarm.description}</p>
                          </div>
                          <div>
                        ...
                        {/* Technologies */}
                        <div>
                          <h4 className="text-xs font-semibold text-gray-600 mb-1">Technologies</h4>
                          <div className="flex flex-wrap gap-1">
                            {alarm.technologies.map(tech => (
                              <span key={tech} className="px-1 py-0.5 bg-indigo-100 text-indigo-800 text-xs rounded">
                                {tech}
                              </span>
                            ))}
                          </div>
                        ...
                        {/* Hierarchy */}
                        <div>
                          <h4 className="text-xs font-semibold text-gray-600 mb-1">Hierarchy</h4>
                        ...
                        {alarm.acknowledged && (
                          <div className="flex items-center gap-1 p-1 bg-green-50 rounded border border-green-200">
                            <Pin className="w-3 h-3 text-green-600" />
      ...
      {/* Row count */}
      <div className="px-4 py-1 bg-gray-50 border-t border-gray-200 text-xs text-gray-600">
